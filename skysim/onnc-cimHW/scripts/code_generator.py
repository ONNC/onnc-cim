...
def gen_runtime_substitution_hash(schema):
  hash = {
    ...
    'operator_name_capital': schema.name.capitalize(),
  }
  ...
  def io_parameter(prefix, ctype):
    def cb(io_schema):
      if OpSchema.FormalParameterOption.Variadic == io_schema['option']:
        return [
          ',{ctype}* const* {prefix}_{io_name}'.format(prefix=prefix, ctype=ctype, **io_schema),
          ',dim_type {prefix}_{io_name}_ntensor'.format(prefix=prefix, ctype=ctype, **io_schema),
          ',const_dim_type* {prefix}_{io_name}_ndim, const_dim_type* const* {prefix}_{io_name}_dims'.format(prefix=prefix, ctype=ctype, **io_schema)
        ]
      else:
        return [
          ',{ctype}* {prefix}_{io_name}'.format(prefix=prefix, ctype=ctype, **io_schema),
          ',const_dim_type {prefix}_{io_name}_ndim, const_dim_type* {prefix}_{io_name}_dims'.format(prefix=prefix, ctype=ctype, **io_schema)
        ]
    return cb
  flatten = lambda l: [item for sublist in l for item in sublist]
  # ${input_parameters_float}
  hash['input_parameters_float'] = '\n  '.join(flatten(for_io_schema(schema.inputs, io_parameter('input', 'const_element_type'))))
  # ${output_parameters_float}
  hash['output_parameters_float'] = '\n  '.join(flatten(for_io_schema(schema.outputs, io_parameter('output', 'element_type'))))
  # ========== end of Input Output ==============
  ...
  def attr_cb(attr):
    if attr['attr_type'][-1] == 's':
      ...
      return [
        ...
        ',const_dim_type number_of_{attr_name}'.format(**attr),
      ]
    else:
      attr['attr_type'] = attr_type_map[attr['attr_type']]
      return [
...
def gen_interpreter_substitution_hash(schema):
  ...
  def pass_io(prefix):
    def cb(io_schema):
      ...
      if io_schema['option'] == OpSchema.FormalParameterOption.Variadic:
        return [
          ',{prefix}_{io_name}'.format(**io_schema),
  ...
  # ========== end of Attributes ==============
  # special for fastSimWrapper visitor
  if len(schema.inputs) != 0:
    hash['visitor_pass_fastsim_argument'] = '\n  '.join(flatten(for_io_schema(schema.inputs, pass_io('input'))))[1:] + \
                                            '\n  '.join(flatten(for_io_schema(schema.outputs, pass_io('output')))) + \
                                            '\n  '.join(flatten(for_attr(pass_attr)))
  else:
    hash['visitor_pass_fastsim_argument'] = '\n  '.join(flatten(for_io_schema(schema.outputs, pass_io('output'))))[1:] + \
                                            '\n  '.join(flatten(for_attr(pass_attr)))
  return hash
...
if __name__ == '__main__':
  ...
  gen_interpreter(operator_schemas, 'cimHWSimulator.template.hh', 'cimHWSimulator.func.template.hh', output_dir + 'cimHWSimulator.hh')
  gen_interpreter(operator_schemas, 'cimHWSimulator.template.cc', 'cimHWSimulator.func.template.cc', output_dir + 'cimHWSimulator.cc')
  gen_interpreter(operator_schemas, 'abstractOpFactory.template.hh', 'abstractOpFactory.func.template.hh', output_dir + 'abstractOpFactory.hh')
  gen_interpreter(operator_schemas, 'abstractOpFactory.template.cc', 'abstractOpFactory.func.template.cc', output_dir + 'abstractOpFactory.cc')
  gen_interpreter(operator_schemas, 'fastSimWrapper.template.hh', 'fastSimWrapper.class.template.hh', output_dir + 'fastSimWrapper.hh')
  gen_interpreter(operator_schemas, 'fastSimWrapper.template.cc', 'fastSimWrapper.class.template.cc', output_dir + 'fastSimWrapper.cc')
