...
// InterpreterPass
//===----------------------------------------------------------------------===//
InterpreterPass::InterpreterPass(TargetBackend* pBackend, std::unique_ptr<char[]> pInputMem,
                                 std::function<void(const Tensor&, const void*)> pOutputListener, unsigned int pVerbose)
  : m_pBackend(pBackend)
...
Pass::ReturnType InterpreterPass::runOnModule(Module& pModule)
{
  ...
  for (ComputeOperand* co : pModule.getComputeOperands()) {
    if (ComputeMemOperand* mem = dyn_cast<ComputeMemOperand>(co)) {
      ...
      if (mem->isInput()) {
        // XXX: Multiple inputs
        m_pInterpreter->getComputeEngine().m_ATable[v] = m_pInputMem.get();
      } else if (mem->isWeight()) {
        // XXX
        FloatTensor* t                                 = static_cast<FloatTensor*>(v);
        m_pInterpreter->getComputeEngine().m_ATable[v] = t->getValues().data();
        weight_memory_size += t->getValues().size() * sizeof(FloatTensor::ValueList::value_type);
      } else {
        mem_start[co->getValue()]  = mem->start();
        mem_length[co->getValue()] = mem->length();
        internal_memory_size = std::max(internal_memory_size, static_cast<uint64_t>(mem->start()) + mem->length());
      }
    }
  }
  if (m_Verbose >= 1) {
  ...
  if (m_Verbose >= 4) {
    ...
    size_t            ptr_len = 8;
    os << "[v4] " << std::setw(val_len) << "Value" << sep << std::setw(ptr_len) << "[offset"
       << "   " << std::setw(ptr_len) << ")end"
       << "   " << std::setw(ptr_len) << "size" << std::endl;
    os << "[v4] " << std::setfill('-') << std::setw(val_len) << '-' << "-+" << std::setw(ptr_len * 3 + 2 * 2 + 3) << '-'
    ...
    for (ComputeOperator& cm : *pModule.getRootComputeGraph()) {
      ...
      for (int i = 0; i < cm.getNumOfOutputs(); ++i) {
        ...
        os << "[v4] " << std::setw(val_len) << v->getName() << sep << std::internal << std::hex << std::setfill('0')
           << "0x" << std::setw(ptr_len) << mem_start[v] << (mem_end == internal_memory_size ? '*' : ' ') << "0x"
           << std::setw(ptr_len) << mem_end << ' ' << std::right << std::dec << std::setfill(' ') << std::setw(ptr_len)
           << mem_length[v] << std::endl;
      }
    }
    outs() << os.str();
  }
  ...
  int fail = posix_memalign(reinterpret_cast<void**>(&heap), 16, internal_memory_size);
  ...
  for (ComputeOperand* co : pModule.getComputeOperands()) {
    if (ComputeMemOperand* mem = dyn_cast<ComputeMemOperand>(co)) {
      if (mem->isOutput() || mem->isInternal()) {
        m_pInterpreter->getComputeEngine().m_ATable[co->getValue()] = heap + mem->start();
      }
    }
  }
...
Pass::ReturnType InterpreterPass::runInterpreter(Module& pModule)
{
  ...
  m_pInterpreter->getComputeEngine().m_pContext = ONNC_RUNTIME_init_runtime();
  ...
  for (ComputeOperator& cm : *pModule.getRootComputeGraph()) {
    if (OutputOperator* out = dyn_cast<OutputOperator>(&cm)) {
      for (int i = 0; i < out->getNumOfInputs(); ++i) {
        ...
        const auto* const  data  = m_pInterpreter->getComputeEngine().m_ATable[value];
        m_OutputListener(static_cast<const Tensor&>(*value), data);
      }
    }
  }

  ONNC_RUNTIME_shutdown_runtime(m_pInterpreter->getComputeEngine().m_pContext);

  return Pass::kModuleNoChanged;
}

} // namespace of cim_internal
} // namespace of onnc
