...
#include "CIMBackend.h"
#include "CIMInterpreter.h"
#include "TargetInfo/CIMTargetInfo.h"
#include "TargetInfo/CIMTargetMemInfo.h"
...
// for interprete, will be refactor in the future
#include "CIMInternal.h"
...
// CIMBackend
//===----------------------------------------------------------------------===//
CIMBackend::CIMBackend(const TargetOptions& pOptions)
  : TargetBackend(pOptions) { 
  m_pMemInfo = std::make_unique<CIMTargetMemInfo>();
}

void CIMBackend::addTensorSel(PassManager& pPM)
{
...
void CIMBackend::addOnncIrOptimization(PassManager& pPM, OptimizationOptions& options)
{
  ...
  options.defaultEnable(OptimizationOption::eliminate_identity);
  //options.defaultEnable(OptimizationOption::split_conv_by_channel);

  TargetBackend::addOnncIrOptimization(pPM, options);
}

void CIMBackend::addTensorSched(PassManager& pPM)
{
...
void CIMBackend::addMemAlloc(PassManager& pPM)
{
  // Input: Module
  ...
  // Do interprete!!
  if(options().getCIMRunInterpreter())
  {
    // read the input tensor from specified file
    std::unique_ptr<char[]> input;
    auto result = cim_internal::readTensor(options().getCIMInterpreterInput());
    if (result) {
      input = std::move(result.m_Data);
    }

    // set the output proxy, defaut is to std
    auto writeProxy = [this]() {
      if (options().getCIMInterpreterOutput() != TargetOptions::CIMInterpreterDefaultOutputName) {
        return cim_internal::TensorWriteProxy{options().getCIMInterpreterOutput()};
      }
      return cim_internal::TensorWriteProxy{};
    }();

    // Run interpret!
    pPM.add<cim_internal::InterpreterPass>(
      this,
      std::move(input),
      std::move(writeProxy),
      options().getVerboseLevel()
    );
  }
}

void CIMBackend::addCodeEmit(PassManager& pPM, const Path& pOutput)
{
  // static cim::CodeEmitVisitor ceVisitor;
  // pPM.add<CodeEmit>(ceVisitor);
}

Interpreter *CIMBackend::createTargetInterpreter() const {
  return new CIMInterpreter(options().getCIMConfig());
}

void CIMBackend::RegisterLowers(LowerRegistry& pRegistry) const
{
...
//===----------------------------------------------------------------------===//
TargetBackend* CreateCIMBackend(const TargetOptions& pOptions)
{
  return new CIMBackend(pOptions);
}

extern "C" void InitializeCIMONNCBackend()
{
  onnc::TargetRegistry::RegisterTargetBackend(getTheCIMTarget(),
      CreateCIMBackend);
}
