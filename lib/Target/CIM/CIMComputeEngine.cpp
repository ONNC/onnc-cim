...
// CIMComputeEngine
//===----------------------------------------------------------------------===//
CIMComputeEngine::CIMComputeEngine(const onnc::Path &configFilePath) noexcept
  : m_configFilePath(configFilePath)  
{}

void CIMComputeEngine::compute(const Initializer& co) {}
void CIMComputeEngine::compute(const InputOperator& co) {}
void CIMComputeEngine::compute(const OutputOperator& co) {}

void CIMComputeEngine::compute(const Abs& pOp) {
  // Prepare input
  ...
  // Call to Runtime
  m_cimSim.cimHW_Abs_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
...
void CIMComputeEngine::compute(const Acos& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const Add& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const And& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const ArgMax& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const ArgMin& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Asin& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Atan& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const AveragePool& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_AveragePool_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const BatchNormalization& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_BatchNormalization_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
...
void CIMComputeEngine::compute(const Cast& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Ceil& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Clip& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Concat& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const Constant& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const Conv& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_Conv_float(
            reinterpret_cast<void*>(&m_configFilePath.native()),
            reinterpret_cast<float*>(input_X), input_X_ndim, input_X_dims, reinterpret_cast<float*>(input_W),
            input_W_ndim, input_W_dims, reinterpret_cast<float*>(input_B), input_B_ndim, input_B_dims,
            reinterpret_cast<float*>(output_Y), output_Y_ndim, output_Y_dims, auto_pad, dilations, number_of_dilations,
            group, kernel_shape, number_of_kernel_shape, pads, number_of_pads, strides, number_of_strides);
}


void CIMComputeEngine::compute(const ConvTranspose& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ConvTranspose_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
...
void CIMComputeEngine::compute(const Cos& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const DepthToSpace& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_DepthToSpace_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const Div& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Dropout& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Elu& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Equal& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Exp& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Expand& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Flatten& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Floor& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const GRU& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Gather& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Gemm& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_Gemm_float(
            reinterpret_cast<void*>(&m_configFilePath.native()),
            reinterpret_cast<float*>(input_A), input_A_ndim, input_A_dims, reinterpret_cast<float*>(input_B),
            input_B_ndim, input_B_dims, reinterpret_cast<float*>(input_C), input_C_ndim, input_C_dims,
            reinterpret_cast<float*>(output_Y), output_Y_ndim, output_Y_dims, alpha, beta, transA, transB);
}


void CIMComputeEngine::compute(const GlobalAveragePool& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_GlobalAveragePool_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const GlobalLpPool& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_GlobalLpPool_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const GlobalMaxPool& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_GlobalMaxPool_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const Greater& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const HardSigmoid& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_HardSigmoid_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const Hardmax& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Identity& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const InstanceNormalization& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_InstanceNormalization_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(input_scale)
...
void CIMComputeEngine::compute(const LRN& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const LSTM& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const LeakyRelu& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_LeakyRelu_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const Less& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Log& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const LogSoftmax& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_LogSoftmax_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const LpNormalization& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_LpNormalization_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const LpPool& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const MatMul& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Max& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const MaxPool& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const MaxRoiPool& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_MaxRoiPool_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
...
void CIMComputeEngine::compute(const Mean& pOp) {
  // Prepare input
  int32_t input_data_0_ntensor = pOp.getNumOfInputs() - 0;
  void *input_data_0[input_data_0_ntensor];
  for (int i = 0; i < input_data_0_ntensor; ++i) input_data_0[i] = m_ATable[pOp.getInput(0 + i)];
...
void CIMComputeEngine::compute(const Min& pOp) {
  // Prepare input
  int32_t input_data_0_ntensor = pOp.getNumOfInputs() - 0;
  void *input_data_0[input_data_0_ntensor];
  for (int i = 0; i < input_data_0_ntensor; ++i) input_data_0[i] = m_ATable[pOp.getInput(0 + i)];
...
void CIMComputeEngine::compute(const Mul& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Multinomial& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_Multinomial_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const Neg& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Not& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Or& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const PRelu& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Pad& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Pow& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const RNN& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const RandomNormal& pOp) {
  // Prepare input
  ...
  // Call to Runtime
  m_cimSim.cimHW_RandomNormal_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
...
void CIMComputeEngine::compute(const RandomNormalLike& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_RandomNormalLike_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const RandomUniform& pOp) {
  // Prepare input
  ...
  // Call to Runtime
  m_cimSim.cimHW_RandomUniform_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
...
void CIMComputeEngine::compute(const RandomUniformLike& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_RandomUniformLike_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const Reciprocal& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_Reciprocal_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const ReduceL1& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const ReduceL2& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const ReduceLogSum& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceLogSum_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const ReduceLogSumExp& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceLogSumExp_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const ReduceMax& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceMax_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const ReduceMean& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceMean_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const ReduceMin& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceMin_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const ReduceProd& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceProd_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const ReduceSum& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceSum_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const ReduceSumSquare& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ReduceSumSquare_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
    , reinterpret_cast<float *>(output_reduced)
...
void CIMComputeEngine::compute(const Relu& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Reshape& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Selu& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Shape& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Sigmoid& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Sin& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Size& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Slice& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Softmax& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Softplus& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Softsign& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const SpaceToDepth& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_SpaceToDepth_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const Split& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Sqrt& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Squeeze& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Sub& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Sum& pOp) {
  // Prepare input
  int32_t input_data_0_ntensor = pOp.getNumOfInputs() - 0;
  void *input_data_0[input_data_0_ntensor];
  for (int i = 0; i < input_data_0_ntensor; ++i) input_data_0[i] = m_ATable[pOp.getInput(0 + i)];
...
void CIMComputeEngine::compute(const Tan& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Tanh& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Tile& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const TopK& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Transpose& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_Transpose_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
...
void CIMComputeEngine::compute(const Unsqueeze& pOp) {
  // Prepare input
  const Tensor *input_data_t = pOp.getInput(0);
  void *input_data = m_ATable[input_data_t];
  int32_t input_data_ndim = input_data_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_Unsqueeze_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_data)
    , input_data_ndim, input_data_dims
...
void CIMComputeEngine::compute(const Upsample& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const Xor& pOp) {
  // Prepare input
  const Tensor *input_A_t = pOp.getInput(0);
  void *input_A = m_ATable[input_A_t];
  int32_t input_A_ndim = input_A_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const ATen& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const Affine& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const ConstantFill& pOp) {
  // Prepare input
  ...
  // Call to Runtime
  m_cimSim.cimHW_ConstantFill_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const Crop& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const GRUUnit& pOp) {
  // Prepare input
...
void CIMComputeEngine::compute(const GivenTensorFill& pOp) {
  // Prepare input
  ...
  // Call to Runtime
  m_cimSim.cimHW_GivenTensorFill_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_shape)
    , input_shape_ndim, input_shape_dims
...
void CIMComputeEngine::compute(const ImageScaler& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ImageScaler_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const MeanVarianceNormalization& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_MeanVarianceNormalization_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const ParametricSoftplus& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ParametricSoftplus_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
...
void CIMComputeEngine::compute(const Scale& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
...
void CIMComputeEngine::compute(const ScaledTanh& pOp) {
  // Prepare input
  const Tensor *input_input_t = pOp.getInput(0);
  void *input_input = m_ATable[input_input_t];
  int32_t input_input_ndim = input_input_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ScaledTanh_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_input)
    , input_input_ndim, input_input_dims
    , reinterpret_cast<float *>(output_output)
...
void CIMComputeEngine::compute(const ThresholdedRelu& pOp) {
  // Prepare input
  const Tensor *input_X_t = pOp.getInput(0);
  void *input_X = m_ATable[input_X_t];
  int32_t input_X_ndim = input_X_t->getNumOfDimensions();
  ...
  // Call to Runtime
  m_cimSim.cimHW_ThresholdedRelu_float(
    reinterpret_cast<void*>(&m_configFilePath.native())
    , reinterpret_cast<float *>(input_X)
    , input_X_ndim, input_X_dims
    , reinterpret_cast<float *>(output_Y)
