...
//////////
// options for cim backend
static cl::opt<std::string>
OptCIMInterpreterInput("cim-interpreter-input", cl::kLong, cl::kOptional,
 cl::kValueRequired, cl::desc("File to be used for interpretation"), cl::about(g_About));

static cl::opt<std::string>
OptCIMInterpreterOutput("cim-interpreter-output", cl::kLong, cl::kOptional,
 cl::kValueRequired, cl::desc("File to store the result of interpretation"), cl::about(g_About));

static cl::opt<std::string>
OptCIMConfig("cim-config", cl::kLong, cl::kOptional,
 cl::kValueRequired, cl::init(""), cl::desc("Json file for cim device parameters"), cl::about(g_About));
// end of options for cim backend
//////////
...
int main(int pArgc, char* pArgv[])
{
  ...
  // cim backend command options
  if(OptCIMInterpreterInput.hasOccurrence())
  {
    onnc.options().target().setCIMRunInterpreter(true);
    if(!exists(OptCIMInterpreterInput))
    {
      errs() << Color::MAGENTA << "Fatal" << Color::RESET
           << ": input file not found: " << OptCIMInterpreterInput << std::endl
           << "Use --cim-interpreter-input to specify input file" << std::endl;
      return EXIT_FAILURE;
    }
    onnc.options().target().setCIMInterpreterInput(OptCIMInterpreterInput);
    if(OptCIMInterpreterOutput.hasOccurrence())
      onnc.options().target().setCIMInterpreterOutput(OptCIMInterpreterOutput);

    // set up all cim related parameters
    if(OptCIMConfig.hasOccurrence())
      onnc.options().target().setCIMConfig(OptCIMConfig);
  }
  // end of cim backend command options
